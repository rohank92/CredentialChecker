...
def getUserChoice():
    print ("This tool leverages the HIBP database to find compromised email accounts and passwords.\nCurrent Paste sources : Pastebin, Pastie, Slexy, Ghostbin, QuickLeak, JustPaste, AdHocUrl,\n OptOut.\nWhen using the password checker (option 2) your password is NOT transmitted, it is\n hashed (SHA1) AND only the first 5 characters are used to verify if it is available online.\n You can verify this in the source code.\n")
    while True:
        user_input = raw_input("USAGE: Press Enter after your choice\n\nEnter\t'1' for email check \n\t'2' for password check \n\t'3' to EXIT\n")
        if user_input == '1':
            user_account = raw_input("\nEnter email ID to check\n")
